...
def flag(ARGS):
    ...
    if ARGS.brute_force:
        for prefix in flag_prefixes:
            ...
            # XOR
            # TODO: Automatically find key after searching, by XORing first prefix character with first found character
            for key in range(1, 256):
                search.append(xor(prefix, key))
            info("- XOR")
            
            # ROT
            # TODO: Automatically find key after searching, by subtracting first found character from first prefix character
            for key in range(1, 256):
...
def setup(subparsers):
    ...
    parser.add_argument('-b', '--brute-force', action='store_true', help="Try to brute-force XOR and ROT encodings with 256 attempts (may take some time)")
    parser.add_argument('-c', '--context', type=int, help="Limit amount of characters to show around match, useful if a file with very long lines matches")
